...
router.get("/", (req, res) => {
  ...
  const appointments = db
    .prepare(`
      ...
      WHERE (@doctorId = '' OR CAST(a.doctor_id AS TEXT) = @doctorId)
...
router.post("/", (req, res) => {
  ...
  const patient = db.prepare("SELECT id FROM patients WHERE id = ?").get(Number(req.body.patient_id));
  const doctor = db.prepare("SELECT id FROM doctors WHERE id = ?").get(Number(req.body.doctor_id));
